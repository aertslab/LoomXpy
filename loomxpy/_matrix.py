...
class DataMatrix():
    ...
    def _validate_data_matrix(data_matrix, feature_names, observation_names):
        if feature_names is None:
            raise Exception("The given `data_matrix` argument is a NumPy 2D matrix. Hence, the `feature_names` argument should be provided.")
        if observation_names is None:
            raise Exception("The given `data_matrix` argument is a NumPy 2D matrix. Hence, the `observation_names` argument should be provided.")
        if len(observation_names) != data_matrix.shape[0] or len(feature_names) != data_matrix.shape[1]:
            raise Exception(f"""
...
and {len(feature_names)} features to data matrix with {data_matrix.shape[0]} rows and {data_matrix.shape[1]} columns.
    ...
    @staticmethod
    def _normalize_data_matrix_from_numpy_matrix(data_matrix, feature_names, observation_names) -> sparse.csr_matrix:
    ...
    @staticmethod
    def _normalize_data_matrix_from_sparse_matrix(data_matrix, feature_names, observation_names) -> sparse.csr_matrix:
        DataMatrix._validate_data_matrix(
            data_matrix=data_matrix,
            feature_names=feature_names,
            observation_names=observation_names
